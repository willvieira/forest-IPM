...
must_haves:
  truths:
    ...
    - "parameters(mod, draw = 'mean') returns an ipm_parameters object with $species_params, $draw_type, and $seed fields — each species entry has $fixed, $random_effects, and $draw_id"
  ...
  key_links:
    ...
    - from: "R/project.R project()"
      to: "R/kernel.R mkKernel() + R/BasalArea_competition.R size_to_BAcomp()"
      via: "project() loops over timesteps: each step calls mkKernel(), multiplies K %*% Nvec, updates competition via size_to_BAcomp()"
