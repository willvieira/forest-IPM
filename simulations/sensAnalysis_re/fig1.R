...
sim_pars <- read_csv('simulations/sensAnalysis_re/output/simulation_pars.csv') |>
  mutate(
...
out <- parse_number(
    dir(
      'simulations/sensAnalysis_re/output/', pattern = '.RDS'
    )
  ) |>
  sort() |>
  map_dfr(
    ~readRDS(
      paste0('simulations/sensAnalysis_re/output/sim_', .x, '.RDS')
    )[['year_summ']] |>
    bind_cols(sim = .x)
  ) |>
...
pdf(
  file = 'simulations/sensAnalysis_re/dist_lambda_N_BA.pdf',
  width = 16,
  height = 12
)
...
pdf(
  file = 'simulations/sensAnalysis_re/importance.pdf',
  width = 12,
  height = 10
)
...
out_dt |>
  ...
  group_by(parvr) |>
  reframe(
    q = quantile(impvr, probs = c(0.01, 0.5, 0.99)),
    qq = c('l', 'm', 'u')
  ) |>
  pivot_wider(
    names_from = qq,
...
pdf(
  file = 'simulations/sensAnalysis_re/importance_summ.pdf',
  width = 14,
  height = 10
)
out_dt |>
