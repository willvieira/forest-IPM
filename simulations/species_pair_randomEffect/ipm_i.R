...
pars_sp2[['random_effects']] <- rnorm(
  n = 3, # growth, mort, and recruitment
  mean = 0,
  sd = c(
    pars_sp2[['growth']]['sigma_plot'],
    pars_sp2[['mort']]['sigma_plot'],
    pars_sp2[['rec']]['sigma_plot']
  )
)
...
K0_sp2 = mkKernel(
  ...
  Prec = prec,
  pars = pars_sp2,
  plot_random = pars_sp2$random_effects
)
...
for(t in 2:pars$n_time)
{
  ...
  N_sp2$Nvec <- K0_sp2$K %*% N0_sp2$Nvec
